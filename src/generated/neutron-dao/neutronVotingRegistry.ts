...
export interface NeutronVotingRegistrySchema {
  ...
  execute: AddVotingVaultArgs | RemoveVotingVaultArgs | UpdateConfigArgs;
  [k: string]: unknown;
}
...
export interface RemoveVotingVaultArgs {
...
export class Client {
  ...
  removeVotingVault = async(sender:string, args: RemoveVotingVaultArgs, fee?: number | StdFee | "auto", memo?: string, funds?: Coin[]): Promise<ExecuteResult> =>  {
          if (!isSigningCosmWasmClient(this.client)) { throw this.mustBeSigningClient(); }
    return this.client.execute(sender, this.contractAddress, { remove_voting_vault: args }, fee || "auto", memo, funds);
  }
