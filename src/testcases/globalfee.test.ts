...
const EXPECTED_UNTRN_MIN_FEE = '0.900000000000000000';
const EXPECTED_IBC_UATOM_MIN_FEE = '0.026000000000000000';
const EXPECTED_IBC_USDC_MIN_FEE = '0.250000000000000000';
...
describe('Globalfee module', () => {
  ...
  beforeAll(async () => {
    ...
    context.client = await SigningCosmWasmClient.connectWithSigner(NEUTRON_NODE_RPC_ADDR, context.wallet);
  ...
  test('get min fee params', async () => {
    const resp = await axios.get(
      NEUTRON_NODE_REST_ADDR + `/gaia/globalfee/v1beta1/params`,
    );
    ...
    expect(minFees).toEqual(expect.arrayContaining([
      expect.objectContaining({ 'amount': EXPECTED_UNTRN_MIN_FEE, 'denom': UNTRN_DENOM }),
      expect.objectContaining({ 'amount': EXPECTED_IBC_UATOM_MIN_FEE, 'denom': IBC_UATOM_DENOM }),
      expect.objectContaining({ 'amount': EXPECTED_IBC_USDC_MIN_FEE, 'denom': IBC_USDC_DENOM })
    ]));
  ...
  describe('test minfee', () => {
    test('a bit less than minfee is rejected', async () => {
      ...
      const fee1 = +ntrnMinFee.amount - 0.000100; // sub 100untrn from min value
    ...
    test('a bit more than minfee is accepted', async () => {
      ...
      const fee1 = +ntrnMinFee.amount + 0.000100; // add 100untrn to min value
      const fee2 = Math.floor(fee1 * 200000); // multiply to gas
      const fee: StdFee = { amount: [{ amount: fee2.toString(), denom: UNTRN_DENOM }], gas: '200000' }
