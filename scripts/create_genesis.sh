...
jq --arg utc_date "$UTC_DATE" --arg sequence "$WASM_SEQUENCES" 'del(.validators) | .genesis_time = $utc_date | .app_state.wasm.sequences[1].value = $sequence' $SNAPSHOT_INPUT > $GENESIS_OUTPUT
